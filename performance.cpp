...
void avx512_openmp_test(uint16_t array[ARRAY_SIZE][VECTOR_SIZE], uint16_t target_key) {
    __m512i target_vec = _mm512_set1_epi16(target_key);

    int match_found = 0;

    #pragma omp parallel for shared(match_found)
    for (int i = 0; i < ARRAY_SIZE; i++) {
...
int main() {
    ...
    printf("\nAVX-512 with OpenMP Test:\n");
    measure_time(avx512_openmp_test, array, target_key, "AVX-512 with OpenMP Test");
