...
#define ARRAY_SIZE 1000000   // 数组中有 1,000,000 个向量
...
// 场景 2：逐键比较（所有键）
void key_to_key_test(uint16_t (*array)[VECTOR_SIZE], uint16_t target_key, int *indices) {
    for (int i = 0; i < ARRAY_SIZE; i++) {
        int index = indices[i];
        for (int j = 0; j < VECTOR_SIZE; j++) {
...
// 场景 3：逐键比较（单键）
void single_key_test(uint16_t (*array)[VECTOR_SIZE], uint16_t target_key, int *indices) {
    for (int i = 0; i < ARRAY_SIZE; i++) {
        int index = indices[i];
...
// 测试运行时间
double measure_time(void (*test_func)(uint16_t (*array)[VECTOR_SIZE], uint16_t, int *),
                    uint16_t (*array)[VECTOR_SIZE],
                    uint16_t target_key, int *indices,
...
int main() {
    // 动态分配内存并显式类型转换
    uint16_t (*array)[VECTOR_SIZE] = (uint16_t (*)[VECTOR_SIZE])malloc(ARRAY_SIZE * sizeof(*array));
